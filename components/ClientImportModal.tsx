...
import { X, Upload, FileSpreadsheet, ArrowLeft, Check, Loader2, AlertTriangle, User, ShieldCheck } from 'lucide-react';
...
const ClientImportModal: React.FC<ClientImportModalProps> = ({ isOpen, onClose, onImport, isImporting, userProfile }) => {
  ...
  const runAudit = (data: any[]) => {
    ...
    data.forEach((item, index) => {
      ...
      if (origin && !matchedOriginKey) {
        originErrors.push({ row: index + 1, name: `${item.firstName} ${item.lastName}`, origin, subOrigin, source, error: `L'origine "${origin}" n'existe pas dans le référentiel.` });
        ...
        if (subOrigin && !matchedSubOriginKey) {
          originErrors.push({ row: index + 1, name: `${item.firstName} ${item.lastName}`, origin, subOrigin, source, error: `La sous-origine "${subOrigin}" n'est pas valide pour l'origine "${origin}".` });
          ...
          if (source && !matchedSource) {
            originErrors.push({ row: index + 1, name: `${item.firstName} ${item.lastName}`, origin, subOrigin, source, error: `La source "${source}" n'est pas valide pour "${origin} > ${subOrigin}".` });
          }
        }
      }
    });
  ...
  const handleConfirmImport = async () => {
    const finalData = previewData.map(item => {
      ...
      if (!matchedOriginKey) {
        ...
        if (cleanSubOrigin && !matchedSubOriginKey) {
          cleanSubOrigin = '';
          cleanSource = '';
        } else if (matchedSubOriginKey) {
            ...
            <div className="p-8 space-y-8">
              <div className="grid grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-2">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Lignes</p>
                  <p className="text-3xl font-black text-gray-900">{auditReport.totalRows}</p>
                </div>
                <div className={`p-6 rounded-2xl border shadow-sm space-y-2 ${auditReport.unknownCollaborators.length > 0 ? 'bg-amber-50 border-amber-100' : 'bg-white border-gray-100'}`}>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Collab. Inconnus</p>
                  <p className={`text-3xl font-black ${auditReport.unknownCollaborators.length > 0 ? 'text-amber-600' : 'text-gray-900'}`}>{auditReport.unknownCollaborators.length}</p>
                </div>
                <div className={`p-6 rounded-2xl border shadow-sm space-y-2 ${auditReport.originErrors.length > 0 ? 'bg-red-50 border-red-100' : 'bg-white border-gray-100'}`}>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Erreurs Origines</p>
                  <p className={`text-3xl font-black ${auditReport.originErrors.length > 0 ? 'text-red-600' : 'text-gray-900'}`}>{auditReport.originErrors.length}</p>
                </div>
              </div>
              ...
              {auditReport.unknownCollaborators.length === 0 && auditReport.originErrors.length === 0 && (
