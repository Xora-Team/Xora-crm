      ...
      <button
        ...
        className={`w-full flex items-center justify-between border rounded-[14px] px-4 py-3 text-[14px] transition-all duration-200 ${
          isOpen 
            ...
            : (!value || (multiple && value.length === 0)) ? 'border-gray-200 hover:border-gray-300 shadow-sm' : 'border-indigo-600 shadow-sm'
...
const NumberInput = ({ value, onChange, unit }: { value: any; onChange: (v: number) => void; unit?: string }) => (
  <div className="relative group">
    <input 
      ...
      className={`w-full border rounded-[14px] px-4 py-3 text-[14px] font-bold text-gray-900 outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-50 transition-all shadow-sm ${value ? 'bg-indigo-50/50 border-indigo-600' : 'bg-white border-gray-200'}`}
    />
...
const LongTextField = ({ value, onChange, placeholder = "Saisir ici...", rows = 3 }: any) => (
  <textarea 
    ...
    className={`w-full border rounded-[14px] p-4 text-[14px] font-bold text-gray-900 outline-none focus:border-indigo-600 focus:ring-4 focus:ring-indigo-50 transition-all resize-none shadow-sm ${value ? 'bg-indigo-50/50 border-indigo-600' : 'bg-white border-gray-200'}`}
  />
);
